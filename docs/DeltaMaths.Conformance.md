...
The run also loads [`src/DeltaMaths/Vectors/shader-contract.json`](../src/DeltaMaths/Vectors/shader-contract.json)
and executes one case for every supported `Builtin` or `Helper` overload. The
generated case list must match the manifest exactly; missing or extra cases fail
the run. Regenerate the case source after a contract change with:
...
The generated counts and bundle hash are recorded in the generated
[`shader-conformance-report.md`](shader-conformance-report.md).
